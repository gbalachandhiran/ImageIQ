    ...
    public synchronized String toHTMLString(String htmlElementID) {
        ...
        for (int row = 0; row < data.length; row++) {
            ...
            for (int column = 0; column < columns.length; column++) {
                ...
                result += escapeHtml4(columns[column]) + " "
                        + Utils.getHTMLInputFromType(columnID, data[row][column], true);
            ...
            result +=
                    "<button type=\"button\" class=\"removeButton\" onclick=\"removeDataTableRow(this.parentNode.parentNode, this.parentNode);\" "
                            + ((row == 0) ? "hidden" : "") + ">Remove</button>";
        ...
        result += "<button type=\"button\" onclick=\"addDataTableRow(document.getElementById('" + htmlElementID
                + "'));\">Add</button><br />"; // FIXME replace the getelementbyid with this.previoussibling ?!? http://www.w3schools.com/dom/prop_element_previoussibling.asp
