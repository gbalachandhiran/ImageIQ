...
public class Search {
    ...
    public Search(ServletRequest request) {
        ...
        if (qp != null) {
            ...
            this.selectedProviders.addAll(JSONArray.toCollection(arr, String.class));
        } else {
    ...
    private void mountSimpleSearch(ServletRequest request) {
                                                                      ...
                                                                      // empty...
        {
            ...
            if (!isKeyworded()) {
                ...
                QueryExpressionBuilder exp = new QueryExpressionBuilder(simpleQuery);
    ...
    private void mountAdvancedSearch(ServletRequest request) {
        ...
        useStartDate = Utils.parseCheckBoxValue(request.getParameter("fromDate"));
    ...
    public String getAdvancedQuery() {
        ...
        // patient gender
        if ((patientGender != null) && (!patientGender.isEmpty()) && (!isPatientGenderAll())) {
        ...
        if ((institutionName != null) && (!institutionName.isEmpty())) {
            result = addExtraQueryParam(result, "InstitutionName:(" + institutionName + ")");
        }

        // physician
        if ((physician != null) && (!physician.isEmpty())) {
            result = addExtraQueryParam(result,
                    "(PerformingPhysicianName:(" + physician + ") OR ReferringPhysicianName:(" + physician + "))");
        }
        ...
        if ((studyDateFormat != null) && (!studyDateFormat.isEmpty())) {
            if (studyDateFormat.equalsIgnoreCase("Exact")) {
                if ((exactDate != null) && (!exactDate.isEmpty()))
                    result = addExtraQueryParam(result, "StudyDate:(" + exactDate + ")");
            } else {
                if (studyDateFormat.equalsIgnoreCase("Range")) {
                    String date = "StudyDate:[";

                    if (useStartDate && (startDate != null) && (!startDate.isEmpty())) {
                        date += startDate;
                    } else {
                        date += "0000101";
                    }
    ...
    public List<String> getQueryProviders() {
        List<String> providers = PluginController.getInstance().getQueryProvidersName(true);
    ...
    private int fireSearch(String query) {
        ...
        SearchHolder holder = (SearchHolder) httpSession.getAttribute("dicoogle.web.queryHolder");
        if (holder == null) {
    ...
    private Collection<SearchResult> search(String query, boolean fullRequest) {
        ...
        try {
            ...
            itResults = PluginController.getInstance().query(t, selectedProviders, query, searchParam).get();
    ...
    public boolean isPatientGenderAll() {
        return (patientGender == null) || patientGender.isEmpty() || patientGender.equalsIgnoreCase("All")
                || ((!patientGender.equalsIgnoreCase("Male")) && (!patientGender.equalsIgnoreCase("Female")));
    }

    public boolean isPatientGenderMale() {
        return (patientGender != null) && patientGender.equalsIgnoreCase("Male");
    }

    public boolean isPatientGenderFemale() {
        return (patientGender != null) && patientGender.equalsIgnoreCase("Female");
    }
    ...
    public boolean isExactDate() {
        return (studyDateFormat != null) && (!studyDateFormat.isEmpty()) && studyDateFormat.equalsIgnoreCase("Exact");
    }

    public boolean isRangedDate() {
        return (studyDateFormat != null) && (!studyDateFormat.isEmpty()) && studyDateFormat.equalsIgnoreCase("Range");
    }
