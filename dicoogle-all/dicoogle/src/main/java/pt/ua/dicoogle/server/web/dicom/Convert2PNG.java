...
public class Convert2PNG {
     ...
     */
    @Deprecated
    public static ByteArrayOutputStream DICOM2PNGStream(StorageInputStream dcmStream, int frameIndex, int transformType,
            int transformParam1, float transformParam2) {
        ...
        // NOTE the above compression params are not currently needed because ImageIO already automatically compresses PNG by default, it's depicted on
        // http://bugs.sun.com/bugdatabase/view_bug.do?bug_id=4829970 as a bug but it's more of a "can't set the compression level" problem as it uses the
        // Deflater.BEST_COMPRESSION as default (see line 144 of com.sun.imageio.plugins.png.PNGImageWriter), the code remains comment so if in the future
        // ImageIO allows for compression level set we can set it to the best compression level again
     ...
     */
    public static ByteArrayOutputStream DICOM2PNGStream(StorageInputStream dcmStream, int frameIndex)
            throws IOException {
     ...
     * @throws IOException if the I/O operations on the images fail
     */
    public static ByteArrayOutputStream DICOM2ScaledPNGStream(InputStream inStream, int frameIndex, int width,
     ...
     * @throws IOException if the I/O operations on the images fail
     */
    public static ByteArrayOutputStream DICOM2ScaledPNGStream(StorageInputStream dcmStream, int frameIndex, int width,
            int height) throws IOException {
    ...
    public static BufferedImage scaleImageByWidth(BufferedImage image, int width) {
        ...
        BufferedImage result =
                new BufferedImage(scaledImage.getWidth(null), scaledImage.getHeight(null), BufferedImage.TYPE_INT_RGB); // create the output image
