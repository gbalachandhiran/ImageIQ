...
public class XMLSupport extends DefaultHandler {
    ...
    public void startElement(String uri, String localName, String qName, Attributes attribs) {
        ...
        if (localName.equals("options")) {
            ...
            MoveDestination tmp = new MoveDestination(this.AETitle, this.IP, this.port, this.isPublic.contains("true"),
                    this.description);
        ...
        try {
            ...
            for (i = l.size() - 1; i >= 0; i--) {
                ...
                if (LocalTS.getAccepted()) {
                    ...
                    if (TS[2]) {
                        hd.startElement("", "", "TS", atts);
                        hd.characters(UID.DeflatedExplicitVRLittleEndian.toCharArray(), 0,
                                UID.DeflatedExplicitVRLittleEndian.length());
                        hd.endElement("", "", "TS");
                    }
                    if (TS[3]) {
                        hd.startElement("", "", "TS", atts);
